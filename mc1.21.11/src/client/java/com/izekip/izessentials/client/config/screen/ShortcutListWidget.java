...
/**
 * 12 macro slotunu duzenlenebilir satirlar olarak gosteren liste.
 * Her satir dogrudan calisma kopyasindaki ShortcutEntry nesnesine yazar
 * (bkz. ShortcutConfigScreen - diske sadece "Kaydet" ile yazilir).
 */
public class ShortcutListWidget extends ContainerObjectSelectionList<ShortcutListWidget.Entry> {
	...
	public static class Entry extends ContainerObjectSelectionList.Entry<Entry> {
		...
		private final List<net.minecraft.client.gui.components.AbstractWidget> widgets;
		...
		public void renderContent(GuiGraphics guiGraphics, int mouseX, int mouseY, boolean hovered, float partialTick) {
			...
			guiGraphics.drawString(font, "Slot " + slotNumber, x, y, AuraTheme.TEXT_WHITE);
			guiGraphics.drawString(font, keyMapping.getTranslatedKeyMessage(), x, y + 11, AuraTheme.TEXT_MUTED);

			int enabledWidth = Math.max(enabledBox.getWidth(), 14);
			int commandX = x + INFO_WIDTH + LABEL_WIDTH + GAP * 2;
			int commandWidth = Math.max(40, rowWidth - INFO_WIDTH - LABEL_WIDTH - enabledWidth - GAP * 3);

			labelBox.setX(x + INFO_WIDTH);
			labelBox.setY(y);
			commandBox.setX(commandX);
			commandBox.setY(y);
			commandBox.setWidth(commandWidth);
			enabledBox.setX(x + rowWidth - enabledWidth);
			enabledBox.setY(y);
